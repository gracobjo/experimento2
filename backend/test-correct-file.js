...
async function testCorrectFile() {
  try {
    console.log('🧪 Iniciando prueba con archivo correcto actualizado...');
    
    // Datos de prueba con el archivo correcto
    const testInvoiceData = {
      numeroFactura: "CORRECT-001",
      tipoFactura: "F",
    ...
    console.log('📋 Datos de prueba con archivo correcto');
    ...
    // Crear la factura
    console.log('🔄 Creando factura con archivo correcto...');
    ...
    // Descargar el PDF
    console.log('📥 Descargando PDF con archivo correcto...');
    ...
    fs.writeFileSync('test-correct-file.pdf', pdfResponse.data);
    console.log('📁 PDF guardado como: test-correct-file.pdf');
    ...
    console.log('🎯 Prueba con archivo correcto completada');
    console.log('📁 Archivo generado: test-correct-file.pdf');
    console.log('📏 Tamaño final:', pdfBuffer.length, 'bytes');

  } catch (error) {
    console.error('❌ Error en la prueba con archivo correcto:', error.message);
    if (error.response) {
...
// Ejecutar la prueba con archivo correcto
testCorrectFile(); 