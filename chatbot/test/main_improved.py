        ...
        if date.weekday() < 5:  # Lunes a viernes
            for hour in [9, 10, 11, 12, 16, 17]:  # Horarios disponibles
                available_dates.append({
                    ...
                    'formatted': date.strftime('%A %d de %B a las %H:%M').replace('Monday', 'Lunes').replace('Tuesday', 'Martes').replace('Wednesday', 'Miércoles').replace('Thursday', 'Jueves').replace('Friday', 'Viernes')
                })
...
def handle_appointment_conversation(user_id: str, message: str) -> str:
    ...
    if conv.stage == "collecting_info":
        if not conv.data["fullName"]:
            ...
            # Mapear automáticamente el motivo a un tipo de derecho
            consultation_type = map_consultation_reason_to_type(message.strip())
            conv.data["consultationType"] = consultation_type
            conv.current_question = "date"
            return f"Entiendo. He identificado que tu consulta corresponde al área de **{consultation_type}**.\n\n¿Qué fecha prefieres para tu consulta?\n\n📅 **Selecciona tu fecha y hora:**\n\nSe abrirá un calendario visual donde podrás elegir la fecha y hora que mejor te convenga. Haz clic en la fecha deseada y luego selecciona el horario disponible."
            ...
            try:
                ...
                # Si no es un número, verificar si es una fecha formateada del calendario visual
                if "de" in message and "a las" in message:
                    # Es una fecha del calendario visual
                    try:
                        # Parsear la fecha del formato "Lunes 15 de Julio a las 10:00"
                        import re
                        from datetime import datetime
                        
                        # Extraer día, mes y hora
                        pattern = r'(\w+)\s+(\d+)\s+de\s+(\w+)\s+a\s+las\s+(\d+):(\d+)'
                        match = re.search(pattern, message)
                        
                        if match:
                            day_name, day, month_name, hour, minute = match.groups()
                            ...
                            if month_name in month_map:
                                ...
                                # Crear fecha
                                selected_date = datetime(current_year, month, day, hour, minute)
                                
                                # Verificar que la fecha sea futura
                                if selected_date > datetime.now():
                                    conv.data["preferredDate"] = selected_date.isoformat()
                                    conv.stage = "confirmation"
                                    return create_confirmation_message(conv.data)
                                else:
                                    return "La fecha seleccionada ya ha pasado. Por favor, selecciona una fecha futura."
                            else:
                                return "No pude entender la fecha. Por favor, selecciona una fecha válida."
                        else:
                            return "Formato de fecha no reconocido. Por favor, selecciona una fecha válida."
                    except Exception as e:
                        print(f"[ERROR] Error parsing calendar date: {e}")
                        return "Hubo un problema al procesar la fecha. Por favor, intenta de nuevo."
                else:
                    return "Por favor, responde con el número de la opción que prefieras o selecciona una fecha del calendario."
    ...
    # Confirmación
        ...
        if is_affirmative_response(message):
            ...
            try:
                print(f"[DEBUG] Enviando cita al backend: {conv.data}")
                ...
                appointment_data = {
                    ...
                    "consultationReason": conv.data["consultationReason"],
                    "preferredDate": conv.data["preferredDate"],
                    "consultationType": conv.data["consultationType"],
                    "notes": conv.data.get("notes", ""),
                    "location": conv.data.get("location", "Oficina principal")
                }
                
                print(f"[DEBUG] Datos preparados: {appointment_data}")
                
                response = requests.post(f"{BACKEND_URL}/api/appointments/visitor", json=appointment_data, timeout=10)
...
def map_consultation_reason_to_type(reason: str) -> str:
    """Mapea el motivo de consulta a un tipo de derecho específico"""
    reason_lower = reason.lower().strip()
    
    # Mapeo de motivos a tipos de derecho
    family_keywords = ["divorcio", "separación", "custodia", "hijos", "familia", "matrimonio", "pensión", "alimentos", "adopción", "patria potestad"]
    civil_keywords = ["herencia", "testamento", "propiedad", "inmueble", "contrato", "daños", "responsabilidad", "negligencia", "desahucio"]
    laboral_keywords = ["despido", "trabajo", "laboral", "empleo", "salario", "vacaciones", "horas", "acoso", "accidente", "baja"]
    mercantil_keywords = ["empresa", "sociedad", "comercial", "mercantil", "negocio", "contrato", "deuda", "quiebra", "concursal"]
    penal_keywords = ["delito", "criminal", "penal", "multa", "cárcel", "acusación", "denuncia", "robo", "agresión", "fraude"]
    administrativo_keywords = ["administrativo", "ayuntamiento", "gobierno", "licencia", "permiso", "sanción", "recurso", "funcionario"]
    
    if any(keyword in reason_lower for keyword in family_keywords):
        return "Derecho Familiar"
    elif any(keyword in reason_lower for keyword in civil_keywords):
        return "Derecho Civil"
    elif any(keyword in reason_lower for keyword in laboral_keywords):
        return "Derecho Laboral"
    elif any(keyword in reason_lower for keyword in mercantil_keywords):
        return "Derecho Mercantil"
    elif any(keyword in reason_lower for keyword in penal_keywords):
        return "Derecho Penal"
    elif any(keyword in reason_lower for keyword in administrativo_keywords):
        return "Derecho Administrativo"
    else:
        return "Derecho Civil"  # Por defecto
...
def process_message(text: str, language: str = "es", conversation_history: list | None = None, user_id: Optional[str] = None) -> str:
    ...
    if text.lower().strip() in ["reset", "reiniciar", "limpiar", "nuevo"]:
    ...
    if is_affirmative_response(text):
        ...
        if conversation_history:
            ...
            # Si el último mensaje del asistente mencionaba citas, iniciar conversación
            if last_assistant_message and any(word in last_assistant_message for word in ["cita", "agendar", "programar", "consulta"]):
                active_conversations[user_id] = AppointmentConversation()
                return "¡Perfecto! Te ayudo a agendar tu cita. Para comenzar, necesito algunos datos:\n\n¿Cuál es tu nombre completo?"
