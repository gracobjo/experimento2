...
const EditCasePage = () => {
  ...
  // Cargar clientes, abogados y expediente
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const token = localStorage.getItem('token');
        const [clientsResponse, lawyersResponse, caseResponse] = await Promise.all([
        ...
        const exp = caseResponse.data;
        setFormData({
          title: exp.title || '',
          description: exp.description || '',
          clientId: exp.client?.id || '',
          lawyerId: exp.lawyer?.id || (user?.role === 'ABOGADO' ? user.id : '')
        });
        setError(null);
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Error al actualizar el expediente');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Editar Expediente</h1>
              <p className="mt-2 text-gray-600">
                Modifica los datos del expediente legal
              </p>
            </div>
            <button
